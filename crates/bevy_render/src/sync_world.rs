...
pub type SyncToRenderWorld = SyncToSubWorld<RenderApp>;
...
pub type RenderEntity = SubEntity<RenderApp>;
...
pub(crate) fn entity_sync_system<L: AppLabel>(main_world: &mut World, render_world: &mut World) {
        ...
        for record in pending.drain(..) {
            match record {
                ...
                EntityRecord::ComponentRemoved(main_entity, removal_function) => {
                    let Some(render_entity) = world.get::<SubEntity<L>>(main_entity) else {
                        continue;
                    };
...
mod tests {
    ...
    use bevy_app::{AppLabel, AppLabelBase};
    use bevy_ecs::{
        component::Component,
        entity::Entity,
        lifecycle::{Add, Remove},
    ...
    #[derive(Debug, Default, Clone, Copy, Hash, PartialEq, Eq, AppLabelBase)]
    struct ExtractApp;

    impl AppLabel for ExtractApp {}
    ...
    fn sync_world() {
        ...
        main_world.add_observer(
            |add: On<Add, SyncToSubWorld>, mut pending: ResMut<PendingSyncEntity>| {
