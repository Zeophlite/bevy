...
/// implement the trait for a foreign type you can use a local type as the
/// marker, e.g. the type of the plugin that calls [`ExtractResourcePlugin`].
pub struct ExtractBaseResourcePlugin<L: AppLabel, R: ExtractResource<L, F>, F = ()>(
    PhantomData<(L, R, F)>,
);

pub type ExtractResourcePlugin<R, F = ()> = ExtractBaseResourcePlugin<RenderApp, R, F>;

impl<L: AppLabel, R: ExtractResource<L, F>, F> Default for ExtractBaseResourcePlugin<L, R, F> {
    fn default() -> Self {
        Self(PhantomData)
    }
}

impl<L: AppLabel, R: ExtractResource<L, F>, F: 'static + Send + Sync> Plugin
...
pub fn extract_resource<L: AppLabel, R: ExtractResource<L, F>, F>(
