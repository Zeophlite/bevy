        ...
        text_input_query.iter_mut()
    {
        for edit in text_input_actions.queue.drain(..) {
            if let Err(error) = apply_text_edit(
                ...
                commands.trigger(TextInputEvent::InvalidEdit(error, edit).with(entity));
            }
        }
        ...
        if let Some(mut value) = maybe_value {
            ...
            if value.0 != contents {
                value.0 = contents;
                commands.trigger(TextInputEvent::TextChanged.with(entity));
            }
        }
    }
}
...
pub fn apply_text_edit(
    ...
    match edit {
        ...
        &TextEdit::Scroll { lines } => {
            // TODO: convert lines to pixels OR change Scroll to store pixels
            let pixels: f32 = lines as f32;
...
pub struct TextInputEventContainer {
    /// a
    pub entity : Entity,
    /// b
    pub input : TextInputEvent,
}
...
impl TextInputEvent {
    /// a
    pub fn with(self, entity : Entity) -> TextInputEventContainer {
        TextInputEventContainer { entity, input: self.clone() }
    }
}
