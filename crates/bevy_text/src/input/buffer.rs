...
use bevy_time::Time;
use cosmic_text::{
    skrifa::{prelude::Size, FontRef, MetadataProvider, Tag},
    Buffer, BufferLine, Edit, Editor, Metrics,
};
        ...
        let _ = editor.with_buffer_mut(|buffer| {
                ...
                })
            {
                ...
                *space_advance = font_id_map
                    ...
                    .and_then(|(id, ..)| font_system.get_font(*id, face_info.weight))
                    .and_then(|font| {
                        // TODO: this is rather painful!
                        let face = font.shaper();

                        let font_ref: FontRef = FontRef::new(font.data()).expect("");
                        let location = font_ref
                            .axes()
                            .location([(Tag::new(b"wght"), face_info.weight.0 as f32)]);
                        ...
                        let res = hor_advance as f32 / face.units_per_em() as f32;
                        Some(res)
                    })
                    .unwrap_or(0.0)
